...
export default function ProposalDetailPage() {
  ...
  React.useEffect(() => {
    if (proposal && proposal.status === "SENT" && isLatestProposalVersion(proposal)) {
  ...
  const canRespond = isLatest && (status === "SENT" || status === "VIEWED")
    ...
    <div className="flex flex-col gap-6">
      {!isLatest && latestVersion && (
        <div className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-3 text-sm text-foreground">
          ...
          <Link href={`/portal/proposals/${latestVersion.id}`} className="font-medium underline">
        ...
        <div className="flex flex-col gap-6 lg:col-span-1">
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <ProposalVersionStrip proposal={proposal} basePath="/portal/proposals" />
            ...
            {canRespond && (
              <CardFooter className="flex-col gap-2 border-t">
                <Button className="w-full" onClick={() => respond("APPROVED")}>
                  <CheckIcon />
                  Approve
                </Button>
                <AlertDialog>
