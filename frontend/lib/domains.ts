...
export async function createDomainOrder(
  ...
  const res = await fetch(`${API_URL}/domains/orders`, {
    method: "POST",
    credentials: "include",
...
export async function getMyDomainOrders(): Promise<MyDomainOrder[]> {
  const res = await fetch(`${API_URL}/domains/orders/mine`, { credentials: "include" })
  if (!res.ok) {
...
export async function getAllDomainOrders(): Promise<AdminDomainOrder[]> {
  const res = await fetch(`${API_URL}/domains/admin/orders`, { credentials: "include" })
  if (!res.ok) {
...
export async function getDomainOrder(id: string): Promise<AdminDomainOrder> {
  const res = await fetch(`${API_URL}/domains/admin/orders/${id}`, {
    credentials: "include",
  })
  if (!res.ok) {
